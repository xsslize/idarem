...
- The IDA API is **not thread-safe**, so every database call is marshaled onto IDA's main thread with `ida_kernwin.execute_sync(..., MFF_READ)` and the result handed back to the HTTP worker. This is the core of the plugin.
...
- **Drive IDA (web → IDA)** — toggle **Drive** and your clicks jump IDA's view (`jumpto`), and you can **rename** the selected function and **add comments** at an address from the browser, written straight into the database. `POST /api/goto`, `/api/rename`, `/api/comment`; renames/comments are gated by `ALLOW_WRITE` (set it `False` for read-only).
...
First put Flask in IDA's Python (once): `python -m pip install flask`. Then run the
...
- `Ctrl-Alt-R` toggles the server; open `http://localhost:8765`.
...
1. `python -m pip install flask` into IDA's Python.
...
token, then **Connect**. When the page is served by the plugin, the host field is
...
| You want… | Use | How |
| --- | --- | --- |
| Your own devices, from anywhere | **[Tailscale](https://tailscale.com/)** | Install on both machines; open `http://<workstation-tailnet-ip>:8765`. Nothing is published; works over the internet, not just LAN. |
...
**Set `AUTH_TOKEN` whenever the server is reachable beyond localhost.** The client
sends it as `Authorization: Bearer <token>`; the plugin warns at startup while it's
empty. Responses carry `Access-Control-Allow-Origin: *`, so without a token anyone
who can reach the URL — including a website open in your browser — can read the
database. A token is your gate; a tunnel keeps the port off the public internet.
